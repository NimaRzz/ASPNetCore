...
namespace Project.Application.Common.Validations.Office
{
    public class OfficeValidator
    {
        ...
        public static async Task<ResultDto> ValidateOfficeRequest(object request)
        {
            ...
            {
                result = await ValidateUpdateRequest(updateRequest.Id, updateRequest.Name, updateRequest.ProvinceId, updateRequest.Address, updateRequest.Workdays);
            }
            else if (request is RequestAddOfficeDto addRequest)
            {
                result = await ValidateAddRequest(addRequest.Id, addRequest.Name, addRequest.ProvinceId, addRequest.Address, addRequest.Workdays);
            }

            return result;
        }

        private static async Task<ResultDto> ValidateAddRequest(string id, string name, long province, string address, List<OfficeWorkCalendarCommandsDto> workDays)
        {
        ...
        private static async Task<ResultDto> ValidateUpdateRequest(string id, string name, long province, string address, List<OfficeWorkCalendarCommandsDto> workDays)
        {
            // بررسی اینکه شماره دفتر وارد شده باشد
            if (string.IsNullOrEmpty(id))
            {
